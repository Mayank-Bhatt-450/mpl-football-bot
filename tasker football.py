...
#while True:
    ...
    if (tell_color_dis(img.getpixel((719,730)),( 238 , 241 , 237 ),105.36602868097478) and
        ...
        cor=(round(((fg[0]-495))*2.4053452115812917),round((fg[1])*2.402669632925473))
        
        print ('goal=',cor)
        if side=='l':
            source = requests.get(f'http://192.168.0.100:1817/?message={cor[0]+100},{cor[1]+100},376 1125 574 1581').text
        else:
            print('in')
            source = requests.get(f'http://192.168.0.100:1817/?message={cor[0]}-100,{cor[1]+100},729 1268 482 1788').text 
        

        #source = requests.get(f'http://192.168.0.100:1817/?message={cor[0]},{cor[1]},376 1125 574 1581').text 
        input(f'{cor[0]+10},{cor[1]+10}')
